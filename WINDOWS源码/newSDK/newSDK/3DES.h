...
/*
对密钥进行 加密，解密运算
bType   :类型：加密ENCRYPT，解密DECRYPT
In      :待加密串指针   固定为16字节
Key     :16位密钥      固定输入为16字节
Out     :待输出串指针   固定输出为   16 字节
返回值 说明：
int     :是否加密成功，1：成功，0：失败
...
bType : 类型：加密ENCRYPT，解密DECRYPT
In : 待加密串指针
in_len : 待加密串的长度，同时Out的缓冲区大小应大于或者等于in_len  写死
Key :	写死24字节
Out : 待输出串指针
out_len : 输出缓存大小
返回值 说明：
