...
{
	LOG_WriteLog("start auto sign up process.");
	/* 判断是否需要签到*/
	...
	LOG_WriteLog("start auto sign up process end.");
	return ret;
}
...
int TRANS_SIGNUP(char const *key)
{
	...
	if (recvLen < 0)
	{
		LOG_WriteLog("ERROR : signup process comunication failed.");
	...
	{
		LOG_WriteLog("ERROR : signup process return value failed.");
		CFG_SetProfileString(CFG_IS_USEFUL, "is_useful=0");
		CFG_SetProfileString(CFG_IS_SIGNUP, "is_signup=0");
	...
	char TDKStr[17], TDKSource[33], TDKCheckValue[9];										/* TDK密文 明文 以及 校验值*/
	...
	memset(TDKSource, 0, sizeof(TDKSource));
	...
	{
		LOG_WriteLog("ERROR : start auto sign up error.Check PIK error.");
		CFG_SetProfileString(CFG_IS_USEFUL, "is_useful=0");
		CFG_SetProfileString(CFG_IS_SIGNUP, "is_signup=0");
		return -1;																			/* PIK 校验失败*/
	}
	...
	{
		LOG_WriteLog("ERROR : start auto sign up error.Check MAK error.");
		CFG_SetProfileString(CFG_IS_USEFUL, "is_useful=0");
		CFG_SetProfileString(CFG_IS_SIGNUP, "is_signup=0");
		return -1;																			/* MAK 校验失败*/
	}
...
int TRANS_SIGNOFF()
{
	LOG_WriteLog("start signoff process.");
	int ret = 0;
	PACKSOURCE ps;
	ISO_FILEDS_INIT(&ps);
	char batch[7];
	...
	if (recvLen < 0)
	{
		LOG_WriteLog("ERROR : signoff process comunication failed.");
		return -1;
	}
	/* 处理收到的信息*/
	UNPACKRESULT ur;
	...
	if (strcmp(ur.content[39].data, "00") == 0)													/* 如果签到返回值不是00*/
	{
		...
		LOG_WriteLog("signoff success.");
		return ret = atoi(ur.content[39].data);
	}
	else 
	{
		LOG_WriteLog("ERROR : signoff error.");
		return -1;
	}
	return 0;
}
...
int TRAN_SETTLE()
{
	LOG_WriteLog("start settle process.");
	int ret = 0;
	PACKSOURCE ps;
	ISO_FILEDS_INIT(&ps);
	char batch[7];
	...
	if (recvLen < 0)
	{
		LOG_WriteLog("ERROR : settle process comunication failed.");
		return -1;
	}
	/* 处理收到的信息*/
	UNPACKRESULT ur;
	...
	if (strcmp(ur.content[39].data, "00") == 0)													/* 如果签到返回值不是00*/
	{
		LOG_WriteLog("settle process  success.");
		return ret = atoi(ur.content[39].data);
	}
	else
	{
		LOG_WriteLog("ERROR : settle process failed.");
		return -1;
	}
	return 0;
}
...
{
	LOG_WriteLog("Enter trans process.");
	int signoffTimes = 0;																	/* 交易重试次数 */
signoff:;
	char userpin[20];																		/* 存放用户明文密码*/
	char temp[20];																			/* 配置文件中转 */
	memset(temp, 0, sizeof(temp));
	memset(temp, 0, sizeof(temp));
	/* 得到MAK 明文, PIK明文 */
	...
	if (ps->content[52].is_exist == 1)
	{
		...
		memcpy(userpin, ps->content[52].data, ps->content[52].dataLen);
	...
	tr->txnAmt = txnAmt;
	char log[512];
	memset(log, 0, sizeof(512));
	sprintf(log, "trans process: trans mchTransId [%s] , response code [%s].", mchTransId, respCode);
	LOG_WriteLog(log);
		...
		strcmp(respCode,"a0") == 0 )
	{
		...
		if (signoffTimes >= 4)
		{
			LOG_WriteLog("ERROR :  trans process retry too much .");
			return 0;
		}
		CFG_SetProfileString(CFG_IS_SIGNUP, "is_signup=0");
		...
		//恢复原来的用户密码 
		memset(ps->content[52].data, 0, sizeof(ps->content[52].data));
		memcpy(ps->content[52].data, userpin, sizeof(userpin));
		goto signoff;
	}
	LOG_WriteLog("End trans process. success.");
	return ret;
}
