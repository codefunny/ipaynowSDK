...
added by Principle . nxd
2015.05.21
针对KEY加密解密函数
int KEY_3DES(int bType, const char *In, const char *Key, char* Out);
*/

int KEY_3DES_16(int bType, const char *In, const char *Key, char* Out)
{
	return Run3Des(bType, ECB, In, 16, Key, 16, Out, 16, NULL);
}
...
2015.05.21
针对DATA加密解密函数
int DATA_3DES(int bType, const char *In, const char *Key, char* Out);
*/

int KEY_3DES_8(int bType, const char *In, const char *Key, char* Out)
{
	return Run3Des(bType, ECB, In, 8, Key, 16, Out, 8, NULL);
}
...
2015.05.21
针对DATA加密解密函数
int DATA_3DES(int bType, const char *In, unsigned int in_len, const char *Key, char* Out, unsigned int out_len);
...
int DATA_3DES(int bType, const char *In, unsigned int in_len, const char *Key, char* Out, unsigned int out_len)
{
	...
	if (in_len % 8 == 0)
	{
		ret = Run3Des(bType, ECB, In, in_len, Key, 24, Out, out_len, NULL);
		//if(bType == DECRYPT) RunRsm(Out);
	}
	else{
		int inOutLen;
		char inBig[MAX_IN_LENGTH];
		char* pIn = inBig;
		ret = RunPad(PAD_ISO_1, In, in_len, pIn, &inOutLen);
		ret = Run3Des(bType, ECB, pIn, inOutLen, Key, 24, Out, out_len, NULL);
		//	if (bType == DECRYPT) RunRsm(Out);
		pIn = NULL;
	}
	return ret;
