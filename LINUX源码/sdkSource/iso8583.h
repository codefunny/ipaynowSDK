...
/* 单倍长DES加密解密封装 bType = ENCRYPT or DECRYPT*/
int ISO8583_DES8(int bType, char const *input, char *output, char const *key);
/* 双倍长DES加密解密封装 bType = ENCRYPT or DECRYPT 输入输出都为16进制字符串 32位密钥,16位数据或32位数据*/
...
/* 56域解析 以及 写入 */
//int ISO8583_56FiledGet(char const *input, char const *flag, char const *content);
